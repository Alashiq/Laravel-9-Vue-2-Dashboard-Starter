...
this.formData.branch=response.data.data.branch;
this.formData.account_number=response.data.data.account_number;
this.formData.currency_id=response.data.data.currency_id;
this.formData.name_holder=response.data.data.name_holder;
...
this.validateName_holder();
 if (this.formValidate.name_holder != '') return 0;
        ...
        this.$loading.Start();
            this.$http
            .EditBankAccount(this.$route.params.id,this.formData)
            .then(response => {
                this.$loading.Stop();
                if (response.status == 200) {
                    this.$alert.Success(response.data.message);
